...
class Migration:
    def __init__(self):
        self._src_space_id = None
        self._dst_space_id = None
    ...
    def _find_matched_dst_item_by_src_item(self, src_item_with_dst_ids=None, item_type=None):
        logger.info(f"Look for the matched destination {item_type} {src_item_with_dst_ids.get(name_key)} in "
                    f"{self._dst_space_id}")
        dst_list_items = get_one_type_to_list(item_type=item_type, space_id=self._dst_space_id)
        ...
        if matched_sub_list and len(matched_sub_list) > 1:
            log_raise_value_error(local_logger=logger, item=matched_sub_list,
                                  err=f"For {item_type} {item_name}, more than one item found in {self._dst_space_id}")
    ...
    def _prepare_tag_set(self, src_item=None):
    ...
    def _prepare_project(self, src_item=None):
        logger.info(f"prepare {item_type_projects} {src_item.get(name_key)} for migrating to {self._dst_space_id}")
    ...
    # we do not want to clone the child items first;
    # child items will be created automatically when parent item is created
    def _prepare_library_variable_set(self, src_item=None):
    ...
    def _prepare_feed(self, src_item=None):
        logger.info(f"prepare {item_type_feeds} {src_item.get(name_key)} for migrating to {self._dst_space_id}")
    ...
    def _prepare_account(self, src_item=None):
        logger.info(f"prepare {item_type_accounts} {src_item.get(name_key)} for migrating to {self._dst_space_id}")
    ...
    def _prepare_runbook(self, src_item=None):
        logger.info(f"prepare {item_type_runbooks} {src_item.get(name_key)} {src_item.get(id_key)} for migrating to "
                    f"{self._dst_space_id}: popping {runbook_process_id_key}")
        src_item.pop(runbook_process_id_key, None)
        src_item.pop(published_runbook_snapshot_id_key, None)

    def _clone_item_to_space(self, item_type=None, item_name=None, item_id=None):
        item_badge = item_name if item_name else item_id
        logger.info(f"clone {item_type} {item_badge} from {self._src_space_id} to {self._dst_space_id}")

        full_process = self._type_full_func_dict.get(item_type)
        if full_process:
            logger.info(f"Special full processing for {item_type} {item_name} {item_id} in space {self._dst_space_id} "
    ...
    def _create_item_to_space(self, item_type=None, src_item=None):
        ...
        if src_id_value in self._src_id_vs_dst_id_dict:
            ...
            log_info_print(local_logger=logger,
                           msg=f"{item_type} {src_item_name} {src_id_value} in space {self._src_space_id} has already "
                               f"been cloned to space {self._dst_space_id} as {dst_id_value} in this session; skip it")
            return dst_id_value

        log_info_print(local_logger=logger, msg=f"cloning {item_type} {src_item_name} {src_id_value} in space "
                                                f"{self._src_space_id} to space {self._dst_space_id}")
        logger.info(f"preprocessing {item_type} {src_item_name} {src_id_value} in space {self._src_space_id} with the "
                    f"new references in {self._dst_space_id}...")
        ...
        if prep_process:
            logger.info(f"special preprocessing for {item_type} {src_item_name} {src_id_value} in space "
                        f"{self._src_space_id} with function {prep_process.__name__}")
        ...
        logger.info(f"check if {item_type} {src_item_name} {src_id_value} in space {self._src_space_id} already exists"
                    f" in space {self._dst_space_id}")
        ...
        if dst_item_exist:
            if config.overwrite:
                log_info_print(local_logger=logger,
                               msg=f"destination space {self._dst_space_id} already has {item_type} {src_item_name} "
        ...
        if post_process and (not dst_item_exist or config.overwrite):
            log_info_print(local_logger=logger,
                           msg=f"Additional post-processing for {item_type} {src_item_name} {dst_id_value} in space "
                               f"{self._dst_space_id} with function {post_process.__name__}")
            post_process(src_id=src_id_value, dst_id=dst_id_value)

        return dst_id_value

    def _clone_type_to_space(self, item_type=None):
        if not item_type:
            raise ValueError("item_type must not be empty!")
        log_info_print(local_logger=logger,
                       msg=f"cloning items in type {item_type} from {self._src_space_id} to {self._dst_space_id}")
        src_list_items = self._type_src_list_items_dict.get(item_type)
        if not src_list_items:
            logger.warning(f"{item_type} has no items in {self._src_space_id}, so skip processing it")
            return

        for src_item in src_list_items:
            self._create_item_to_space(item_type=item_type, src_item=src_item)

        logger.info(f"Get the updated {item_type} in {self._dst_space_id}")
        dst_all_items = get_one_type_ignore_error(item_type=item_type, space_id=self._dst_space_id)
    ...
    def _check_broken_item_id(self, string=None):
        ...
        for id_prefix, item_type in item_id_prefix_to_type_dict.items():
            ...
            if re.match(pattern, string):
                log_warn_print(local_logger=logger, msg=f"***** Please remove the broken reference id {string} in "
                                                        f"source space {self._src_space_id} *****")
                return True
        return False
    ...
    def _replace_ids(self, dict_list=None):
    ...
    def _clone_child(self, src_parent_id=None, dst_parent_id=None, child_type=None, child_id_key=None):
        ...
        logger.info(f"clone {parent_type} {parent_name}'s {child_id_key} {src_child_id} from {self._src_space_id} to "
                    f"{self._dst_space_id}")
        ...
        if not src_child:
            logger.warning(f"{src_child_id} does not exist in the memory, so get it from {self._src_space_id}")
            src_child = get_single_item_by_name_or_id(item_type=child_type, item_id=src_child_id,
                                                      space_id=self._src_space_id)
    ...
    def _post_process_project(self, src_id=None, dst_id=None):
        log_info_print(local_logger=logger,
                       msg=f"clone {item_type_deployment_processes} from {item_type_projects} {src_id} in "
                           f"{self._src_space_id} to {dst_id} in {self._dst_space_id}")
        ...
        log_info_print(local_logger=logger,
                       msg=f"clone {item_type_variables} from {item_type_projects} {src_id} in {self._src_space_id} "
                           f"to {dst_id} in {self._dst_space_id}")
    ...
    def _post_process_library_variable_set(self, src_id=None, dst_id=None):
        log_info_print(local_logger=logger,
    ...
    def _post_process_runbook(self, src_id=None, dst_id=None):
        log_info_print(local_logger=logger,
    ...
    def _post_process_tenant_variables(self, src_id=None, dst_id=None):
        log_info_print(local_logger=logger,
    ...
    def _full_process_tags(self, item_type=None, item_name=None, item_id=None):
        log_info_print(local_logger=logger,
                       msg=f"special full process - clone {item_type} {item_name} {item_id} from {self._src_space_id} "
                           f"to {self._dst_space_id}")
        ...
        dst_tag_set = self._find_matched_dst_item_by_src_item(src_item_with_dst_ids=src_tag_set,
                                                              item_type=item_type_tag_sets)
        if dst_tag_set:
            logger.info(f"{item_type_tag_sets} {dst_tag_set.get(name_key)} exists in {self._dst_space_id}, "
            ...
            logger.info(f"{item_type_tag_sets} {dst_tag_set.get(name_key)} does not exist in {self._dst_space_id}, "
    ...
    def _load_types(self, fake_space=False):
        ...
        for item_type in item_types_inside_space:
            ...
            if fake_space:
                logger.info(f"Loading {item_type} in source fake space {self._src_space_id} from the local file...")
                src_list_items = get_list_items_from_file(item_type=item_type, space_id=self._src_space_id)
            # for cloning space to space on the same Octopus server
            else:
                logger.info(f"Loading {item_type} from source space {self._src_space_id}...")
                src_list_items = get_one_type_to_list(item_type=item_type, space_id=self._src_space_id)
        ...
        if fake_space and not actual_src_space_id:
            log_raise_value_error(local_logger=logger, err=f"Could not find an actual space id inside the fake space "
                                                           f"{self._src_space_id}")
        return actual_src_space_id if actual_src_space_id else self._src_space_id
    ...
    def _initialize_maps(self, src_space_id=None, dst_space_id=None, fake_space=False):
        log_info_print(local_logger=logger,
        ...
        self._src_space_id = src_space_id
        self._dst_space_id = dst_space_id
        ...
        actual_src_space_id = self._load_types(fake_space=fake_space)

        if fake_space:
            self._src_id_vs_dst_id_dict[actual_src_space_id] = dst_space_id
        else:
            self._src_id_vs_dst_id_dict[src_space_id] = dst_space_id
    ...
    def _save_space_map(self):
        ...
        local_file = get_local_single_item_file(item_name=space_map + underscore_sign + current_time,
                                                item_type=item_type_migration, space_id=self._dst_space_id)
